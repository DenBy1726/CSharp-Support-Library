...
namespace Calc.Generate
{
    ...
    /// значение нелинейной функции от пезультатов генератора LFSR
    ...
    class Random : System.Security.Cryptography.RandomNumberGenerator, BitGenerator
    {
        ...
        public Int32 Get(int min, int max)
        {
            Int32 retval = 0;
            do
            {
                retval = (Int32)(NextRandomValueU() / 2);
            }
            while (_bitGenerator.GetBit() == false);
            retval = min + (retval % (max - min));
            return retval;
        }
        public UInt32 Get(uint max)
        {
            UInt32 retval = 0;
            do
            {
                retval = NextRandomValueU();
            }
            while (_bitGenerator.GetBit() == false);
            retval = Convert.ToUInt32((retval % (max)));
            return retval;
        }

        public UInt32 Get(int max)
        {
            UInt32 retval = 0;
            do
            {
        ...
        public BigInteger GetBig(uint maxByteLength = 100)
        {
            ...
            byte[] bigStream = new byte[maxByteLength];
            GetBytes(bigStream);
